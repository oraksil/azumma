...
func TestSignalingUseCaseNewOffer(t *testing.T) {
	mockRunningGame := &models.RunningGame{Id: 1, Orakki: &models.Orakki{Id: "orakki1"}}
	mockRunningGameRepo := new(MockRunningGameRepository)
	mockRunningGameRepo.On("FindById", mock.Anything).Return(mockRunningGame, nil)

	mockSignalRepo := new(MockSignalingRepository)
	mockSignalRepo.On("Save", mock.Anything).Return(mock.Anything, nil)

	mockSdpAnswer := models.SetupAnswer{PeerId: "peer-1", Answer: "sdp answer..."}
	mockMsgSvc := new(MockMessageService)
	mockMsgSvc.On("Request", mock.Anything, models.MSG_SETUP_WITH_NEW_OFFER, mock.Anything, 5*time.Second).Return(mockSdpAnswer, nil)

	// when
	useCase := SignalingUseCase{
		RunningGameRepo: mockRunningGameRepo,
		SignalingRepo:   mockSignalRepo,
		MessageService:  mockMsgSvc,
	}

	const sdpString string = `{
		"type": "offer",
	...
	connectionInfo, err := useCase.NewOffer(mockRunningGame.Id, 1, sdpString)
